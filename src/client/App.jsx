...
import { initStarAnimation, starBackgroundStyles } from "./styles/starBackground";
...
const App = () => {
  ...
  useEffect(() => {
    if (theme === 'dark') {
      ...
      setTimeout(() => {
        ...
        const cleanupFn = initStarAnimation(backgroundCanvas, starsCanvas, milkyWayCanvas, animationRef);
  ...
  return (
    <>
      ...
      <div className={`page-background ${isTransitioning ? 'transitioning' : ''}`}></div>
      
      <div style={{...commonStyles.appShell.page, color: colors.text}}>
        {isDark && (
          <div className={`star-background-container ${isTransitioning ? 'transitioning' : ''}`} style={starBackgroundStyles.canvasContainer}>
            <canvas id="backgroundCanvas" style={{...starBackgroundStyles.canvas, ...starBackgroundStyles.backgroundCanvas}}></canvas>
            <canvas id="starsCanvas" style={starBackgroundStyles.canvas}></canvas>
            <canvas id="milkyWayCanvas" style={starBackgroundStyles.canvas}></canvas>
          </div>
        )}
        <Navbar />
        <SubNavbar />
